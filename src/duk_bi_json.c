...
void duk_bi_json_parse_helper(duk_context *ctx,
	...
	/* nothing now */
#endif
	js_ctx->recursion_limit = DUK_USE_JSON_DEC_RECURSION_LIMIT;
	DUK_ASSERT(js_ctx->recursion_depth == 0);
...
void duk_bi_json_stringify_helper(duk_context *ctx,
	...
	if (flags == 0 &&
		 ...
		 * necessary.
		 */
		DUK_ASSERT(DUK_USE_JSON_ENC_RECURSION_LIMIT >= DUK_JSON_ENC_LOOPARRAY);
	...
	js_ctx->recursion_limit = DUK_USE_JSON_ENC_RECURSION_LIMIT;
	DUK_ASSERT(js_ctx->recursion_depth == 0);
